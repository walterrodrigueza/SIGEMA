...
passport.use('local.signin', new LocalStrategy({
  ...
  if (rows.recordsets.length > 0) {
    ...
    if (validPassword) {
      ...
      global.usuario = user;
      console.log('Variable Global Usuario',global.usuario )

      done(null, user, req.flash('success', 'Bienvenido ' + global.usuario.id_usuario));
...
passport.use('local.signup', new LocalStrategy({
  ...
  try{
    ...
    const result = await pool.query("EXECUTE [dbo].[SIGEMA_Usuario_Crear] '"+global.usuario.id_usuario+"','"+id_usuario+"','"+nombre+"','"+primer_apellido+"','"+segundo_apellido+"','"+correo+"','"+tel_oficina+"','"+tel_celular+"','"+rol+"','"+newUser.password+"','"+contraseña+"'");
